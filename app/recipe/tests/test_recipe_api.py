...
def image_upload_url(recipe_id):
    """Get image upload URL for a given recipe id."""
...
class ImageUploadTests(TestCase):
    ...
    def test_upload_image(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            # image_file je samo placeholder: ovde treba da sacuvamo
            # konkretnu sliku
            img = Image.new('RGB', (10, 10))
            img.save(image_file, format='JPEG')

            # Vracamo citac na pocetak fajla
            image_file.seek(0)

            # Saljemo sliku u payload (multipart format)
            payload = {'image': image_file}
            res = self.client.post(url, payload, format='multipart')

            # kada refresh recept iz baze, u odgovoru mora da bude slika
